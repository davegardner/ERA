#!/usr/bin/env python3
"""
Configurable ERA5 climatology processor with adjustable temporal and spatial resolution.
Allows memory-efficient processing by reducing output resolution.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Process ERA5 data into climatological average with configurable resolution',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Current 15GB RAM - recommended
  python3 process_climate_configurable.py --temporal 3 --spatial 1
  
  # Offshore routing - very fast
  python3 process_climate_configurable.py --temporal 6 --spatial 2
  
  # Maximum detail (requires 32GB RAM)
  python3 process_climate_configurable.py --temporal 1 --spatial 1
  
  # Quick test with minimal resources
  python3 process_climate_configurable.py --temporal 24 --spatial 4

Presets:
  --preset coastal    : 3-hourly, 0.25° (9.8 GB, 5 min)
  --preset offshore   : 6-hourly, 0.5°  (2.1 GB, 2.5 min)
  --preset planning   : daily, 1.0°     (1.1 GB, 0.6 min)
  --preset maximum    : hourly, 0.25°   (27.5 GB, 15 min)
        """
    )
    
    parser.add_argument('--temporal', type=int, default=3,
                        help='Temporal resolution in hours (1, 3, 6, 12, 24). Default: 3')
    parser.add_argument('--spatial', type=int, default=1,
                        help='Spatial coarsening factor (1, 2, 4, 8). Default: 1 (0.25°)')
    parser.add_argument('--years', type=str, default='1940-1945',
                        help='Year range to process (e.g., "1940-1945"). Default: 1940-1945')
    parser.add_argument('--preset', type=str, choices=['coastal', 'offshore', 'planning', 'maximum'],
                        help='Use preset configuration (overrides --temporal and --spatial)')
...
def apply_preset(args):
    ...
    presets = {
        'coastal': {'temporal': 3, 'spatial': 1},
        'offshore': {'temporal': 6, 'spatial': 2},
        'planning': {'temporal': 24, 'spatial': 4},
        'maximum': {'temporal': 1, 'spatial': 1},
    }
    
    if args.preset:
        args.temporal = presets[args.preset]['temporal']
        args.spatial = presets[args.preset]['spatial']
...
def main():
    ...
    print(f"📊 ERA5 Climatology Processor (Configurable Resolution)")
    print(f"=" * 70)
    ...
    if not files:
        print(f"❌ No data files found in era5_data/ for years {year_start}-{year_end}")
        return
    ...
    for file_idx, file in enumerate(files):
        ...
        for i in range(n_timesteps):
            ...
            # Skip if not on our temporal resolution
            if hr % args.temporal != 0:
                continue
            
            # Calculate index in output array
            time_idx = (doy - 1) * hours_per_day + (hr // args.temporal)
            ...
            # Progress indicator
            if (i + 1) % 2000 == 0:
                print(f"    {i+1}/{n_timesteps} timesteps", end='\r')
        
        print(f"    {n_timesteps}/{n_timesteps} timesteps - Done!")
        ds.close()
    ...
    if args.output:
        nc_out = f"{args.output}.nc"
        grib_out = f"{args.output}.grb2"
    else:
    ...
    try:
        subprocess.run(["cdo", "-f", "grb2", "copy", nc_out, grib_out], 
