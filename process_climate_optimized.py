#!/usr/bin/env python3
"""
Optimized version: Process entire file at once, then accumulate.
Much faster than processing timestep-by-timestep.
"""
...
print("📊 Building climatological 'typical year' (optimized)...")
...
for file_idx, file in enumerate(files):
    ...
    # Get time info for all timesteps at once
    ...
    print(f"    Accumulating {len(dayofyear)} timesteps...")
    for i in range(len(dayofyear)):
        ...
        # Accumulate
        u10_sum[time_idx] += u10_data[i]
        v10_sum[time_idx] += v10_data[i]
        count[time_idx] += 1
        
        # Progress indicator every 1000 timesteps
        if (i + 1) % 1000 == 0:
            print(f"      {i+1}/{len(dayofyear)} timesteps processed", end='\r')
    
    print(f"      {len(dayofyear)}/{len(dayofyear)} timesteps processed")
    
    ds.close()
    
    # Print memory usage
    mem = psutil.virtual_memory()
    print(f"    Memory: {mem.percent:.1f}% ({mem.used / 1024 / 1024 / 1024:.1f} GB / {mem.total / 1024 / 1024 / 1024:.1f} GB)")
